...
static void Test_normal_output_plans() {
  ...
  CHECK(object.reserved_intermediate_files.size() == 1);
  ...
  CHECK(keep_source_collision.reserved_intermediate_files.size() == 1);
...
static void Test_atomic_reservation_collision_and_cleanup() {
  ...
  const std::vector<std::string> owned = config.reserved_intermediate_files;
...
static void Test_preprocess_state_reserves_nothing() {
  COMPILER_CONFIG config;
  std::string error;
...
int main() {
  ...
  Test_preprocess_state_reserves_nothing();
  if (failures != 0)
