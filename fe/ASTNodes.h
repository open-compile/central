...
public:
  ...
  Json::Value jsonGen() const override {
    Json::Value root;
    root["name"] =
      getTypeName() + this->m_DELIM + name + (isArray ? "(Array)" : "");
...
public:
  shared_ptr<StatementList> child = make_shared<StatementList>();
...
public:
  ...
  NStructDeclaration(shared_ptr<NIdentifier> id,
                     shared_ptr<VariableList> arguments)
...
public:
  ...
  NIfStatement(shared_ptr<NExpression> cond, shared_ptr<NBlock> blk,
               shared_ptr<NBlock> blk2 = nullptr)
...
public:
  ...
  NStructMember(shared_ptr<NIdentifier> structName,
                shared_ptr<NIdentifier> member)
...
public:
  ...
  NArrayInitialization(shared_ptr<NVariableDeclaration> dec,
...
public:
  ...
  NStructAssignment(shared_ptr<NStructMember> member,
                    shared_ptr<NExpression> exp)
