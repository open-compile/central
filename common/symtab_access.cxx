...
T * Symtab_get(IDX idx) {
    AssertThat(File()->Tables()->Get_table((T*) NULL) != NULL, ("table should not be NULL"));
    AssertThat(File()->Tables()->Get_table((T*) NULL)->Get(idx) != NULL, ("item should not be NULL"));
...
template PU_INFO *   Symtab_get<PU_INFO_IDX, PU_INFO, PU_INFO_TABLE> (PU_INFO_IDX idx);
...
//================================================================================
template BOOL Symtab_is_flags_set<TY_IDX, TY, TY_TABLE> (TY_IDX idx, UINT64 flag_opt);
template BOOL Symtab_is_flags_set<ARB_IDX, ARB, ARB_TABLE> (ARB_IDX idx, UINT64 flag_opt);
 ...
 */
template class GLOBAL_SYMTAB_ACCESS<TY_IDX, TY>;
template class GLOBAL_SYMTAB_ACCESS<ARB_IDX, ARB>;
template class GLOBAL_SYMTAB_ACCESS<TYLIST_IDX, TYLIST>;
template class GLOBAL_SYMTAB_ACCESS<PU_IDX, PU>;
template class RELATED_SYMTAB_ACCESS<ST_IDX, ST, TABLE_KIND_ST>;
template class RELATED_SYMTAB_ACCESS<PREG_IDX, PREG, TABLE_KIND_PREG>;
template class GLOBAL_SYMTAB_ACCESS<PU_INFO_IDX , PU_INFO>;
template class GLOBAL_SYMTAB_ACCESS<STR_IDX, const char>;