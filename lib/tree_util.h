...
public:
  ...
  tree(
    const T &);                                 // constructor setting given element as head
  ...
  operator=(const tree<T, tree_node_allocator> &);   // copy assignment
  tree<T, tree_node_allocator> &
  operator=(tree<T, tree_node_allocator> &&);        // move assignment
  ...
  void
  merge(sibling_iterator, sibling_iterator, sibling_iterator, sibling_iterator,
  ...
  iterator
  lowest_common_ancestor(const iterator_base &, const iterator_base &) const;
  ...
  public:
    bool
    operator()(const typename tree<T, tree_node_allocator>::iterator_base &one,
...
template<class T, class tree_node_allocator>
tree<T, tree_node_allocator> &
tree<T, tree_node_allocator>::operator=(tree<T, tree_node_allocator> &&x) {
  if (this != &x) {
    head->next_sibling = x.head->next_sibling;
    feet->prev_sibling = x.head->prev_sibling;
    x.head->next_sibling->prev_sibling = head;
...
template<class T, class tree_node_allocator>
void
tree<T, tree_node_allocator>::copy_(const tree<T, tree_node_allocator> &other) {
  clear();
...
template<class T, class tree_node_allocator>
void
tree<T, tree_node_allocator>::erase_right_siblings(const iterator_base &it) {
  if (it.node == 0) return;
...
template<class T, class tree_node_allocator>
void
tree<T, tree_node_allocator>::erase_left_siblings(const iterator_base &it) {
  if (it.node == 0) return;
...
tree<T, tree_node_allocator>::end_fixed(const iterator_base &pos,
                                        unsigned int dp) const {
  assert(
    1 == 0); // FIXME: not correct yet: use is_valid() as a temporary workaround
  tree_node *tmp = pos.node;
...
template<class T, class tree_node_allocator>
typename tree<T, tree_node_allocator>::sibling_iterator
tree<T, tree_node_allocator>::replace(
...
template<typename iter>
iter
tree<T, tree_node_allocator>::reparent(iter position, sibling_iterator begin,
...
template<class T, class tree_node_allocator>
tree<T, tree_node_allocator>
tree<T, tree_node_allocator>::move_out(iterator source) {
  tree ret;
...
template<class T, class tree_node_allocator>
void
tree<T, tree_node_allocator>::merge(sibling_iterator to1, sibling_iterator to2,
...
template<class T, class tree_node_allocator>
void
tree<T, tree_node_allocator>::sort(sibling_iterator from, sibling_iterator to,
                                   bool deep) {
  std::less<T> comp;
...
template<typename iter, class BinaryPredicate>
bool
tree<T, tree_node_allocator>::equal_subtree(const iter &one_, const iter &two_,
                                            BinaryPredicate fun) const {
...
template<class T, class tree_node_allocator>
tree<T, tree_node_allocator>
tree<T, tree_node_allocator>::subtree(sibling_iterator from,
                                      sibling_iterator to) const {
  assert(
    from != to); // if from==to, the range is empty, hence no tree to return.
...
template<class T, class tree_node_allocator>
unsigned int
tree<T, tree_node_allocator>::number_of_children(const iterator_base &it) {
...
template<class T, class tree_node_allocator>
unsigned int
tree<T, tree_node_allocator>::iterator_base::number_of_children() const {
